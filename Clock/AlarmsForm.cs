			...
			if (lbAlarms.Items.Count > 0)
			{
				if (lbAlarms.SelectedItem != null)
				{
					...
					if (addAlarm.ShowDialog() == DialogResult.OK)
					{
						...
						lbAlarms.Items[lbAlarms.SelectedIndex] = new Alarm(addAlarm.Alarm);
					}
				}
					else MessageBox.Show(this, "Выберете будильник", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
			}
